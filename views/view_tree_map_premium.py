...
from (select DISTINCT(_interval) as i, count(_interval) as c_i, sum(cb_mov2_id) as s_m
      from (SELECT cb_premium_id,
                   cb_mov2_id,
                   case
                       when cb_premium_id <= -0.10 then '<=-10%'
                       when cb_premium_id > -0.10 and cb_premium_id <= -5 then '-10%~-5%'
                       when cb_premium_id > -0.5 and cb_premium_id <= 0 then '-5%~0'
                       when cb_premium_id > 0 and cb_premium_id <= 0.1 then '0~10%'
                       when cb_premium_id > 0.1 and cb_premium_id <= 0.3 then '10%~30%'
                       when cb_premium_id > 0.3 and cb_premium_id <= 0.5 then '30%~50%'
                       when cb_premium_id > 0.5 and cb_premium_id <= 0.8 then '50%~80%'
                       when cb_premium_id > 0.8 and cb_premium_id <= 1.5 then '80%~150%'
                       when cb_premium_id > 1.5 then '>150%' end
                       as _interval,
                   case
                       when cb_premium_id <= -0.10 then 1
                       when cb_premium_id > -0.10 and cb_premium_id <= -5 then 2
                       when cb_premium_id > -0.5 and cb_premium_id <= 0 then 3
                       when cb_premium_id > 0 and cb_premium_id <= 0.1 then 4
                       when cb_premium_id > 0.1 and cb_premium_id <= 0.3 then 5
                       when cb_premium_id > 0.3 and cb_premium_id <= 0.5 then 6
                       when cb_premium_id > 0.5 and cb_premium_id <= 0.8 then 7
                       when cb_premium_id > 0.8 and cb_premium_id <= 1.5 then 8
                       when cb_premium_id > 1.5 then 9 end
                       as _interval_idx
            from changed_bond)
      GROUP by _interval
...
FROM (
         ...
         from (select *
               from (SELECT DISTINCT c.*
                     from changed_bond c WHERE cb_premium_id*100 > :start and cb_premium_id*100 <= :end
                     order by cb_mov2_id DESC
                     limit :up_size) 
               UNION
               select *
               from (SELECT DISTINCT c.*
                     from changed_bond_view c WHERE cb_premium_id*100 > :start and cb_premium_id*100 <= :end
...
order by _sign desc, abs(cb_mov2_id) DESC
        ...
        return '可转债涨跌分布', \
               views.nav_utils.build_analysis_nav_html('/view_tree_map_premium.html'), \
               html
