...
def getContent(code):
    code = common.rebuild_stock_code(code)
    listing_date = get_listing_date(code)

    # 当前时间
    current = time.strftime("%Y%m%d", time.localtime())
    data = ak.stock_zh_a_daily(symbol=code, start_date=listing_date, end_date=current,
                                                  adjust="qfq")
    print(data)
...
def get_listing_date(code):
    url = "https://money.finance.sina.com.cn/bond/info/SZ" + code + ".html"
    ...
    for div in list(divs):
        attr_class = div.attrs['class']
        if 'blk02' not in attr_class:
            continue
        else:
            find = div
            break
    if find is None:
        print("在页面中没有找到指定的html元素")
        raise Exception("在页面中没有找到指定的html元素")
    # 上市时间 fixme 写死了, 页面结构有变化需要改
    listing_date = list(list(list(list(soup.find_all('div'))[22])[3].tbody.children)[40])[1].text
    return listing_date.replace('-', '')
...
class HttpHandler(BaseHTTPRequestHandler):
    def _response(self, path, args):
        ...
        # 获取可转债上市时间(必须从新浪那边取)
        # https://money.finance.sina.com.cn/bond/info/SZ123108.html
...
<head>
    ...
    <script src="view/echarts.min.js"></script>
    <style>
        *{margin:0;padding:0}
        html,body{
...
<body>
    ...
    def do_GET(self):
        path, args = urllib.parse.urlparse(self.path)
...
# httpd = HTTPServer(('127.0.0.1', 7788), HttpHandler)
# httpd.serve_forever()
...
def handle_client(client_socket):
    ...
    if ret:
        if ret.group(1) == 'view_trend.html':
            # 返回浏览器数据
            # 设置返回的头信息 header
